package com.github.benhaixiao.commons.redis.pool;
...
public class ShardedJedisSentinelPool extends Pool<ShardedJedis> {
	...
	private static final Logger log = LoggerFactory.getLogger(ShardedJedisSentinelPool.class);
	...
	/**
	 * @deprecated starting from Jedis 3.0 this method won't exist. Resouce cleanup should be done using @see
			...
			while (!fetched && sentinelRetry < MAX_RETRY_SENTINEL) {
				for (String sentinel : sentinels) {
					...
					try {
						...
						log.warn("Cannot connect to sentinel running @ {}. Trying next one.", hap);
					} finally {
						if (jedis != null) {
							jedis.close();
						}
				...
				if (null == master) {
					try {
						log.error("All sentinels down, cannot determine where is {} master is running... sleeping 1000ms, Will try again.",
								masterName);
						Thread.sleep(1000);
	...
	protected static class ShardedJedisFactory implements PooledObjectFactory<ShardedJedis> {
		...
		public void destroyObject(PooledObject<ShardedJedis> pooledShardedJedis) throws Exception {
			...
			for (Jedis jedis : shardedJedis.getAllShards()) {
				try {
					try {
						if(hasConnected(jedis)) jedis.quit();
					} catch (Exception e) {
		...
		public boolean validateObject(PooledObject<ShardedJedis> pooledShardedJedis) {
			try {
				...
				for (Jedis shard : jedis.getAllShards()) {
					// Add by zhuchengbao(shard.getClient().getSocket() != null) 当ShardedJedis里的某个Jedis从未连接过，则不进行检测
					if (hasConnected(shard) && !shard.ping().equals("PONG")) {
						return false;
					}
				}
				return true;
		...
		/**
		 * jedis是否有连接过
		 * PS : SharedJedis里只是初始化了Jedis实例，但是并没有创建连接，只有在使用到了才会创建连接
		 * @param jedis
		 * @return true 为有连接，否为没连接过
		 */
		private boolean hasConnected(Jedis jedis){
			return jedis.getClient().getSocket() != null;
		}
	}
			...
			while (running.get()) {
				...
				try {
					jedis.subscribe(new JedisPubSubAdapter() {
						@Override
						public void onMessage(String channel, String message) {
							log.info("Sentinel {}:{} published: {}.", host, port, message);
							...
							if (switchMasterMsg.length > 3) {
								...
								if (index >= 0) {
									...
									log.info("Ignoring message on +switch-master for master name {}, our monitor master name are [{}]",
											switchMasterMsg[0], sb);
								}
					...
					if (running.get()) {
						log.error("Lost connection to Sentinel at {}:}{}. Sleeping 5000ms and retrying.", host, port);
						try {
